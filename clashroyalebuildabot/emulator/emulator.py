...
class Emulator:
    ...
    def _get_width_and_height(self):
        ...
        # Try multiple times to get window size
        max_attempts = 3
        for attempt in range(max_attempts):
            try:
                ...
                logger.info(f"Successfully got window size: {width}x{height}")
                return width, height
            except Exception as e:
                logger.warning(f"Attempt {attempt + 1} failed to get window size: {e}")
                if attempt < max_attempts - 1:
                    time.sleep(2)  # Wait before retry
                else:
                    # Fallback to default resolution
                    logger.warning("Using default resolution 1920x1080")
                    return 1920, 1080
